...
class MarketDataServicer(market_data_pb2_grpc.MarketDataServiceServicer):
    ...
    def StreamCandles(self, request, context):
        """Stream candles in real-time"""
        symbol = request.symbol.replace('-', '/')
        interval = request.interval_seconds if request.interval_seconds else 60
        print(f"[gRPC Stream] Starting StreamCandles for {symbol} every {interval}s")
        ...
        while context.is_active():
            try:
                ...
                if not df.empty:
                    ...
                    print(f"[gRPC Stream] Yielding candle {symbol}: close={row['close']:.2f}")
                    yield candle
            except Exception as e:
        ...
        print(f"[gRPC Stream] StreamCandles ended for {symbol}")
